                    ...
                    if author_start_index > -1:
                        ...
                        if f'u{author_id}' in authors:
                            ...
                            if date_start_index > -1:
                                ...
                                date_end_index = stripped_line.find('"', date_start_index)
                                date_string = stripped_line[date_start_index:date_end_index]
                                date = datetime.strptime(date_string, "%Y-%m-%d %H:%M")

                            postid_start_index = stripped_line.find(' id="')
                            if postid_start_index > -1:
                                postid_start_index = postid_start_index + 5
                                postid_end_index = stripped_line.find('"', postid_start_index)
                                post_id = stripped_line[postid_start_index:postid_end_index]
                            ...
                            for paragraph_number, paragraph in enumerate(root.findall('paragraph')):
                                for sentence_number, sentence in enumerate(paragraph.findall('sentence')):
                                    tokens = []
                                    pos = []
